...
def main(args):
    ...
    # 108000
    cmd = "CUDA_VISIBLE_DEVICES={} taskset {} python /cifs/data/tserre_lrs/projects/prj_nih/prj_andrew_holmes/inference/dset_main_inf_forshed.py \
       --expname={} --embname={}"
    ...
    def runner(x):
        ...
        if line != '':
            ...
            csv_name = video_name.split('/')[-1].replace('.mp4', '.csv')
            #import pdb; pdb.set_trace()
            processed_csvs = []
            if os.path.exists(args.base_res_dir + exp_name):
                processed_csvs = os.listdir(args.base_res_dir + exp_name)
            
            if csv_name not in processed_csvs:
                #import pdb; pdb.set_trace()
                #print(cmd.format(gpu, TASKSET_CODES[gpu], exp_name, emb_name))
                os.system(cmd.format(0, TASKSET_CODES[gpu], exp_name, emb_name))
        q.put(gpu)
