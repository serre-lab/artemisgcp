...
def apply_brightness(volume=None):
    '''Apply brightness to the volume consistently
    across frames
    Args:
        volume: '4D Tensor' of shape [num_frames, height, width, n_channels]
    Returns:
        '4D Tensor' of same dtype as :volume
    '''

    volume = tf.image.random_brightness(
        volume,
        max_delta=0.15)

    return volume

def apply_contrast(volume=None):
    '''Apply contrast to the volume consistently
    across frames
    Args:
        volume: '4D Tensor' of shape [num_frames, height, width, n_channels]
    Returns:
    ...
    volume = tf.image.random_contrast(
        volume,
        lower=0.9,
        upper=1.1)
